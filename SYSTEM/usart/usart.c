...
//保存调参或者遥控的数据
//0:Kp 1:Ki 2:Kd 3:Ktp 4:mX 5:mY
float pid_control[pid_num]={0};	
...
void USART_control(u8 Res)
{
	...
	if(sta==6)
	{
		...
		switch(Res)
		{
			...
			case 'x' : sta=4; break;	//mX
			case 'y' : sta=5; break;	//mY
			default  : sta=6; break;
		}
	}
	else
	{
		...
		{
			pid_control[sta]=atof(str);				//字符串转换浮点型
			printf("%f\r\n", pid_control[sta]);
